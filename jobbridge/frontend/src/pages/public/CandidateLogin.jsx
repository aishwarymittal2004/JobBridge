...
export default function CandidateLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.user.role !== "candidate") {
        setError("This account is registered as HR. Use the HR login instead.");
        return;
      }
  ...
  return (
    <main className="mx-auto flex min-h-[80vh] max-w-md flex-col justify-center px-6 py-16">
      ...
      <p className="mt-1 text-sm text-mist-500">Log in to see your career feed.</p>
      ...
      <p className="mt-6 text-center text-sm text-mist-500">
        No account? <Link to="/signup" className="text-signal-cyan hover:underline">Sign up</Link>
      </p>
      <p className="mt-2 text-center text-sm text-mist-500">
        Hiring instead? <Link to="/hr/login" className="text-signal-cyan hover:underline">HR login</Link>
      </p>
    </main>
  );
}
